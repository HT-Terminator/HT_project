...
{
   IIC_Start();
   Write_IIC_Byte(0x78);            //从机地址(OLED外设地址)
	IIC_Wait_Ack();					//等待器件回应
   Write_IIC_Byte(0x00);			//写入操作
	IIC_Wait_Ack();					//等待器件回应
...
void OLED_GPIO_Init()
{
  ...
  { /* Configure GPIO as input mode                                                                         */
    ...
    GPIO_PullResistorConfig(HT_GPIOB, GPIO_PIN_15, GPIO_PR_UP);
...
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;	 //使用PB6和PB7引脚
...
// 	GPIO_SetBits(GPIOB,GPIO_Pin_6|GPIO_Pin_7);	
	...
	delay_ms(800);
	OLED_WR_Byte(0xAE,OLED_CMD);//--display off
	OLED_WR_Byte(0x02,OLED_CMD);//---set low column address  0x02 -> 0x00
	...
	OLED_WR_Byte(0xD8,OLED_CMD);//set area color mode off    将区域颜色模式设置为关闭
